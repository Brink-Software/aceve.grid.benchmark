...
function savePerformanceData(
  ...
  // Performance results directory is in playwright/Tabulator-grid/performance-results
  ...
  data.push({
    ...
    timeMs,
    timestamp,
    testFile: "tabulator-grid.spec.ts",
  });
...
test.describe("Tabulator Grid Tests", () => {
  test.beforeEach(async ({ page }) => {
    // Navigeer naar de Tabulator Grid pagina
    await page.goto("http://localhost:8000/Tabulator-grid");
  });

  test("pagina laadt correct", async ({ page }) => {
    // Check of de pagina geladen is
    await expect(page).toHaveTitle(/Tabulator Grid|Grid Benchmark/i);
  });

  test("Tabulator Grid is zichtbaar", async ({ page }) => {
    // Start tijd meten
    test.slow();
